...
func (f loggerFormatter) needsRequestBody() bool {
	return f.features.needsRequestBody
}

func (f loggerFormatter) needsResponseBody() bool {
	return f.features.needsResponseBody
}

func (f loggerFormatter) needsFormValues() bool {
...
func (f loggerFormatter) render(ctx *gin.Context, record loggerRecord) string {
	return f.renderFormat(func(tag string) (string, bool) {
		return f.resolveTag(tag, ctx, record)
	})
}
