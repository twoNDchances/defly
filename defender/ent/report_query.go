...
type ReportQuery struct {
	...
	withTriggeredByAction *ActionQuery
	withCreatedByDefender *DefenderQuery
...
// QueryTriggeredByAction chains the current query on the "triggered_by_action" edge.
func (rq *ReportQuery) QueryTriggeredByAction() *ActionQuery {
...
// QueryCreatedByDefender chains the current query on the "created_by_defender" edge.
func (rq *ReportQuery) QueryCreatedByDefender() *DefenderQuery {
	query := (&DefenderClient{config: rq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := rq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (rq *ReportQuery) Clone() *ReportQuery {
	...
	return &ReportQuery{
		...
		withTriggeredByAction: rq.withTriggeredByAction.Clone(),
		withCreatedByDefender: rq.withCreatedByDefender.Clone(),
...
// WithTriggeredByAction tells the query-builder to eager-load the nodes that are connected to
// the "triggered_by_action" edge. The optional arguments are used to configure the query builder of the edge.
func (rq *ReportQuery) WithTriggeredByAction(opts ...func(*ActionQuery)) *ReportQuery {
	query := (&ActionClient{config: rq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	rq.withTriggeredByAction = query
	return rq
}

// WithCreatedByDefender tells the query-builder to eager-load the nodes that are connected to
// the "created_by_defender" edge. The optional arguments are used to configure the query builder of the edge.
func (rq *ReportQuery) WithCreatedByDefender(opts ...func(*DefenderQuery)) *ReportQuery {
	query := (&DefenderClient{config: rq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	rq.withCreatedByDefender = query
	return rq
}
...
func (rq *ReportQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Report, error) {
	var (
		...
		_spec       = rq.querySpec()
		loadedTypes = [2]bool{
			rq.withTriggeredByAction != nil,
			rq.withCreatedByDefender != nil,
		}
	)
	...
	if query := rq.withTriggeredByAction; query != nil {
		if err := rq.loadTriggeredByAction(ctx, query, nodes, nil,
			func(n *Report, e *Action) { n.Edges.TriggeredByAction = e }); err != nil {
			return nil, err
		}
	}
	if query := rq.withCreatedByDefender; query != nil {
		if err := rq.loadCreatedByDefender(ctx, query, nodes, nil,
			func(n *Report, e *Defender) { n.Edges.CreatedByDefender = e }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (rq *ReportQuery) loadTriggeredByAction(ctx context.Context, query *ActionQuery, nodes []*Report, init func(*Report), assign func(*Report, *Action)) error {
...
func (rq *ReportQuery) loadCreatedByDefender(ctx context.Context, query *DefenderQuery, nodes []*Report, init func(*Report), assign func(*Report, *Defender)) error {
	ids := make([]uuid.UUID, 0, len(nodes))
	nodeids := make(map[uuid.UUID][]*Report)
	for i := range nodes {
...
func (rq *ReportQuery) querySpec() *sqlgraph.QuerySpec {
	...
	if fields := rq.ctx.Fields; len(fields) > 0 {
		...
		if rq.withTriggeredByAction != nil {
			_spec.Node.AddColumnOnce(report.FieldTriggeredBy)
		}
		if rq.withCreatedByDefender != nil {
			_spec.Node.AddColumnOnce(report.FieldCreatedBy)
		}
	}
