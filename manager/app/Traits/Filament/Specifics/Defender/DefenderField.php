...
trait DefenderField
{
    ...
    public static function setServerEnvironmentVariables()
    {
        ...
        $valueRules = [
            ...
            'SERVER_CONTROLLER_PATH_STATE' => ['required', 'regex:/^[A-Za-z0-9._~-]+(?:\/[A-Za-z0-9._~-]+)*$/', 'not_regex:/(^|\/)\.{1,2}(\/|$)/'],
            'SERVER_CONTROLLER_PATH_GATE' => ['required', 'regex:/^[A-Za-z0-9._~-]+(?:\/[A-Za-z0-9._~-]+)*$/', 'not_regex:/(^|\/)\.{1,2}(\/|$)/'],
            'SERVER_CONTROLLER_PATH_POLICIES' => ['required', 'regex:/^[A-Za-z0-9._~-]+(?:\/[A-Za-z0-9._~-]+)*$/', 'not_regex:/(^|\/)\.{1,2}(\/|$)/'],
            'SERVER_CONTROLLER_PATH_DECISIONS' => ['required', 'regex:/^[A-Za-z0-9._~-]+(?:\/[A-Za-z0-9._~-]+)*$/', 'not_regex:/(^|\/)\.{1,2}(\/|$)/'],
            'SERVER_CONTROLLER_METHOD_CHECK' => ['required', 'in:get,post,put,patch,delete'],
            'SERVER_CONTROLLER_METHOD_INSPECT' => ['required', 'in:get,post,put,patch,delete'],
            'SERVER_CONTROLLER_METHOD_LOCK' => ['required', 'in:get,post,put,patch,delete'],
            'SERVER_CONTROLLER_METHOD_UNLOCK' => ['required', 'in:get,post,put,patch,delete'],
