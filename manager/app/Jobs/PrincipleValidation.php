...
class PrincipleValidation implements ShouldQueue
{
    use Queueable;
    ...
    public function handle(): void
    {
        ...
        try {
            $validation = $this->validatePrinciple($principle);
    ...
    {
        $principlePhase = $this->normalizePhase($principle->getRawOriginal('phase'));

        $principleErrors = [];
        $ruleResults = [];
        $targetCache = [];

        if ($principlePhase === null) {
            $this->pushError($principleErrors, 'principle.phase.invalid', 'Principle phase is invalid.');
        }

        if ($principle->rules->isEmpty()) {
            $this->pushError($principleErrors, 'principle.rules.empty', 'Principle does not have any rules.');
        }

        foreach ($principle->rules as $rule) {
            $ruleResults[] = $this->validateRule($principle, $rule, $principlePhase, $targetCache);
        }

        $rulesFailed = 0;
        $targetsFailed = 0;
        $actionsFailed = 0;
        $errorsTotal = count($principleErrors);
        $targetErrorCounted = [];
        ...
        return [
            ...
            'details' => [
                ...
                'principle' => [
                    'id' => $principle->id,
                    'name' => $principle->name,
                    'phase' => $principlePhase,
                ],
                'summary' => [
                    'rules_total' => $principle->rules->count(),
                    'rules_failed' => $rulesFailed,
                    'targets_failed' => $targetsFailed,
                    'actions_failed' => $actionsFailed,
                    'errors_total' => $errorsTotal,
                ],
                'errors' => $principleErrors,
                'rules' => $ruleResults,
            ],
        ];
    }

    protected function validateRule(Principle $principle, Rule $rule, ?int $principlePhase, array &$targetCache): array
    {
        $errors = [];
        ...
        if (($rulePhase !== null) && ($principlePhase !== null) && ($rulePhase !== $principlePhase)) {
            $this->pushError(
                $errors,
                'rule.phase.principle_mismatch',
                'Rule phase does not match principle phase.',
                ['principle_phase' => $principlePhase, 'rule_phase' => $rulePhase, 'principle_id' => $principle->id]
            );
        }
